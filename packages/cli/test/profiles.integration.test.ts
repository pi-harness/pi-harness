...
describe("packaged profiles", () => {
  ...
  test.each(["default", "development"])(
    ...
    async (profile) => {
      // `everyapi use pi-harness` is the one provisioning step the quickstart names, and it registers everyapi alone. A CLI profile pinning a different provider would leave that step unable to boot the CLI at all, so both shipped profiles have to ask for the same pair the web profile asks for.

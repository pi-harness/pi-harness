...
function errorReport(error: unknown): string {
  ...
  if (unregistered !== null) {
    ...
    const remedy = `The EveryAPI model catalog is not provisioned in PI_CODING_AGENT_DIR (or its PI_AGENT_DIR compatibility alias). Install the EveryAPI CLI with \`curl -fsSL https://dl.everyapi.ai/install.sh | bash\`; \`everyapi use pi-harness\` then registers the catalog there and starts the web console, but it injects the relay key only into the process it starts, so run pih itself through it as \`everyapi use pi-harness -- <pih arguments>\` with a \`pi-harness\` PATH shim that execs pih (the way scripts/pih-local.sh launches the web console), or set PI_HARNESS_PROVIDER and PI_HARNESS_MODEL to a model already registered in that agent directory.`;

...
export function runBoundedCommand(
  argv: readonly string[], cwd: string, timeoutMs: number, maxOutputBytes: number, signal: AbortSignal | undefined,
  options: { env?: NodeJS.ProcessEnv; encoding: "buffer" },
): Promise<{ stdout: Buffer; stderr: Buffer }>;
export function runBoundedCommand(
  argv: readonly string[], cwd: string, timeoutMs: number, maxOutputBytes: number, signal?: AbortSignal,
...
export function runBoundedCommand(
  ...
  if (!Number.isInteger(timeoutMs) || timeoutMs < 1 || timeoutMs > 2 ** 31 - 1) throw new RangeError("Command timeout must be an integer between 1 and 2147483647 ms");

...
describe("SQL Lens panel view", () => {
  ...
  test.each([
    ...
    const view = sqlLensPanelView({
      ...
      timeoutMs: 5_000,
      latest: {
        cwd: "/workspace", database: "data.db", query: "SELECT content FROM users",
        columns: ["content"], rows: [{ content }], truncated, scannedRows: 1,
        rowInventory: { scanned: 1, returned: 1, shown: 1, truncated, displayLimit: 20 },
      },
      limits: {
        queryLength: 65_536, databaseBytes: 268_435_456, rows: 100, columns: 128,
        stringLength: 16_384, resultBytes: 1_048_576, blobPreviewBytes: 256, panelRows: 20,
      },
    });
  ...
  test("keeps arbitrary TEXT and column names while database paths stay strict", () => {
    ...
    expect(sqlLensPanelView({ ...payload, latest: { ...payload.latest, columns: ["id", "no\nte"], rows: [{ id: 1, "no\nte": note }] } }))
      .toMatchObject({ malformed: false, latest: { columns: ["id", "no\nte"], rows: [{ id: 1, "no\nte": note }] } });

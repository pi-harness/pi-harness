...
test.each(["running", "failed", "cancelled"])("does not show a retained pass during %s verification", (status) => {
  const html = renderToStaticMarkup(createElement(PluginPanelCard, { panel: {
    id: "change-verifier-panel", pluginId: "@pi-harness/plugin-change-verifier", title: "Change Verifier",
    data: { status, lastError: status === "running" ? null : "Synthetic provider failed", runs: 1,

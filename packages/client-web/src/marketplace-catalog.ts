...
export const loadMarketplaceCatalog = async (api: Pick<ClientApi, "listMarketplace">, locale = ""): Promise<readonly ClientMarketplacePlugin[]> => {
  const items: ClientMarketplacePlugin[] = [];
  for (let page = 0; ; page += 1) {
    const result = await api.listMarketplace("", "", page, 100, "", locale);

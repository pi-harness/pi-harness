...
function latestView(value: unknown): SqlLensPanelView["latest"] | undefined {
  ...
  const query = typeof source.query === "string" && source.query.length > 0 && source.query.length <= limitsDefaults.queryLength && !source.query.includes("\0")
...
export function sqlLensDisplayText(text: string, multiline = false): string {
  return text.replace(/[\p{Cc}\p{Cf}\p{Cs}\u2028\u2029]/gu, (character) => {
    ...
    return character.split("").map((unit) => "\\u" + unit.charCodeAt(0).toString(16).padStart(4, "0")).join("");
  });
}

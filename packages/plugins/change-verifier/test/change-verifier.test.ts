...
async function createVerifier(options?: { reviewError?: boolean; testError?: boolean; reviewStatus?: unknown; testDurationMs?: unknown; testExitCode?: unknown; afterReview?: () => void; afterTests?: () => void }): Promise<{
  context: Context;
...
describe("change-verifier", () => {
  test("clears old success while a new provider call is running", async () => {
    ...
    const { context, panels, tools } = await createVerifier({ afterReview: () => { observed = panels.snapshot(); } });
    try {
  ...
  test.each(["review", "tests"] as const)("clears prior pass when the next %s provider throws and recovers", async (stage) => {
    let fail = false;
    const reject = () => { if (fail) throw new Error("Synthetic provider failed"); };
  ...
  test.each(["review", "tests"] as const)("rejects session replacement after %s before publishing a gate", async (stage) => {
    ...
    const replace = () => { runtime.session = session("b"); };

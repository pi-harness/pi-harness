...
async function deliver(title: string, message: string, timeoutMs: number, signal: AbortSignal): Promise<NotificationResult> {
  ...
  const run = (argv: string[], env?: NodeJS.ProcessEnv) => runBoundedCommand(argv, process.cwd(), timeoutMs, maxCommandOutputBytes, signal, env === undefined ? {} : { env });
  if (platform === "darwin") {
  ...
  if (platform === "win32") {
    await run(["powershell.exe", "-NoProfile", "-NonInteractive", "-Command", windowsNotificationScript],
      { ...process.env, PI_HARNESS_NOTIFICATION_MESSAGE: message, PI_HARNESS_NOTIFICATION_TITLE: title });
    return { title, message, delivered: true, platform };
  }

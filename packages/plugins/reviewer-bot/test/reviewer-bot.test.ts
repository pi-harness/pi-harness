...
describe("reviewer bot", () => {
  test.skipIf(process.platform === "win32")("stops the sibling Git read when parallel collection fails", async () => {
    const { root, tool } = await fixture(64 * 1024, 15000);
    const bin = join(root, "fixture-bin"), ready = join(root, "ready");
  ...
  test("marks the retained report stale after failure and clears failure on recovery", async () => {
    ...
    await expect(panels.snapshot()).resolves.toMatchObject([{ data: {
      status: "failed", latestStale: true, latest: { status: "pass" },
  ...
  test("reports oversized real Git diff as incomplete rather than an unreadable repository", async () => {
    ...
    await writeFile(join(root, "file.txt"), "large changed line ".repeat(3000));
    await expect(tool.execute("large", {}, undefined, undefined, {} as never)).rejects.toThrow("Git review diff output exceeded 16384 bytes; review is incomplete");
  });

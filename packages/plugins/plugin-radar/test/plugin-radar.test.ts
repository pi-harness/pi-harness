...
describe("plugin radar", () => {
  test("accounts for JSON escaping when limiting returned repositories", async () => {
    vi.stubGlobal("fetch", () => Promise.resolve(Response.json({ total_count: 25, items: Array.from({ length: 25 }, (_, i) => ({
      name: `entry-${i}`, full_name: `acme/entry-${i}`, html_url: `https://github.com/acme/entry-${i}`,
      description: "\u0000".repeat(4_000), stargazers_count: 25 - i,
    })) })));
  ...
  test("bounds model JSON for two large topic responses without losing repository identities", async () => {
    ...
    vi.stubGlobal("fetch", () => {
      const index = request++;
      return Promise.resolve(Response.json({ total_count: 1, items: [{
        name: `large-${index}`, full_name: `acme/large-${index}`, html_url: `https://github.com/acme/large-${index}`,
        description: "界".repeat(300_000), stargazers_count: 10 - index,
      }] }));
    });
  ...
  test("exposes repository metadata and incomplete search status to the model", async () => {
    vi.stubGlobal("fetch", () => Promise.resolve(Response.json({
      total_count: 20, incomplete_results: true,
      items: [{ name: "memory", full_name: "acme/memory", html_url: "https://github.com/acme/memory",
        description: "Persistent memory 测试😀", stargazers_count: 12, language: "TypeScript",
        updated_at: "2026-09-09T00:00:00Z", topics: ["pi-harness-plugin"] }],
    })));
    const { tool } = await fixture();

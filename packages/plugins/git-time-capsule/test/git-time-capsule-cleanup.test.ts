...
const cleanup = vi.hoisted(() => ({ entered: undefined as (() => void) | undefined, release: undefined as Promise<void> | undefined, failure: undefined as Error | undefined }));
...
test.each(["tool", "direct", "direct-cleanup-error"])("reports uncertain workspace after %s restore cleanup interruption", async (mode) => {
  ...
  try {
    ...
    const entered = new Promise<void>((resolve) => { cleanup.entered = resolve; });
    cleanup.release = new Promise<void>((resolve) => { release = resolve; });
    const restore = tools.snapshot().customTools.find((tool) => tool.name === "git_restore")!;
    const pending = mode !== "tool" ? applyCapsule(workspace, join(agentDir, "capsules", name), 15000, controller.signal) :
    ...
    release?.(); cleanup.entered = undefined; cleanup.release = undefined; cleanup.failure = undefined;
    controller.abort();

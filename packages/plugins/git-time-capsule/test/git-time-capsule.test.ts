...
`);
    ...
    try {
      ...
      const operation = action === "restore" ? applyCapsule(workspace, capsule, 500) : tools.snapshot().customTools
        .find((tool) => tool.name === "git_snapshot")!.execute("timeout-zero", {}, undefined, undefined, {} as never);
...
`);
    ...
    const alive = () => {
      try { if (pid === undefined) return false; process.kill(pid, 0); return true; } catch { return false; }
    };
    try {
      await context.plugin(gitTimeCapsulePlugin);
      ...
      await expect(panels.snapshot()).resolves.toMatchObject([{ data: { latest: {
  ...
  test("round-trips non-UTF-8 file contents through real Git", async () => {
    ...
    try {
      await context.plugin(gitTimeCapsulePlugin);
      const result = await tools.snapshot().customTools.find((tool) => tool.name === "git_snapshot")!

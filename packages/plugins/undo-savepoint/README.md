...
Restore preflights every destination before replacing file content, then performs atomic writes per file. It overwrites the files named by the snapshot and recreates missing files; it does not delete files created later. Ignored directory/store entries are skipped and reported. An invalid path or destination discovered during preflight prevents content replacement, although preflight may create missing parent directories. A later I/O failure, cancellation, or session change can leave earlier writes committed: the whole restore is not a transaction and does not roll back completed writes. Stop other workspace writers before restoring. Check the diff and preserve newer changes before confirming an overwrite.
...
Only one tool operation may run at a time. Caller cancellation and plugin disposal reach atomic writes before their commit boundary; an already completed rename cannot be undone. Workspace/session identity is rechecked between asynchronous stages and file writes. An in-flight file write remains bound to its captured workspace, so a session switch cannot redirect it to the new workspace. `diff` compares snapshot content only, does not report added files or permission changes, and classifies unavailable or oversized current files as missing.

...
describe("packaged profiles", () => {
  ...
  test("boots the default production profile without HMR", async () => {
    ...
    expect(names).toContain("@pi-harness/core/plugins/agent-teams");
    expect(names).toContain("@pi-harness/core/plugins/modlens");
    expect(names).toContain("@pi-harness/core/plugins/vision-toolkit");
    expect(names).toContain("@pi-harness/core/plugins/git-time-capsule");
    expect(names).toContain("@pi-harness/core/plugins/dependency-checker");
    expect(names).toContain("@pi-harness/core/plugins/at-file");
    expect(names).toContain("@pi-harness/core/plugins/test-harness");
    expect(names).toContain("@pi-harness/core/plugins/session-insights");
    ...
    expect(names).toEqual(
      expect.arrayContaining([
        "@pi-harness/core/plugins/sql-lens",
        "@pi-harness/core/plugins/docker-sandbox",
        "@pi-harness/core/plugins/mcp-client",
        "@pi-harness/core/plugins/browser-fetch",
        "@pi-harness/core/plugins/browser-session",
        "@pi-harness/core/plugins/yaml-validator",
        "@pi-harness/core/plugins/plugin-dev",
        "@pi-harness/core/plugins/openpets",
        "@pi-harness/core/plugins/skill-guard",
        "@pi-harness/core/plugins/runtime",
        "@pi-harness/core/plugins/context",
        "@pi-harness/core/plugins/token-guard",
        "@pi-harness/core/plugins/fail-logger",
  ...
  test("loads production README tools into the default runtime and writes a current confirmed snapshot", async () => {
    ...
    expect(loaderNames).toContain("@pi-harness/core/plugins/readme-gen");
    ...
    expect((readmeReport.details as { plugins: string[] }).plugins).toEqual(
      expect.arrayContaining(["@pi-harness/core/plugins/readme-gen", "@pi-harness/core/plugins/runtime"]),
    );
  ...
  test("loads the production I18n pair tool into the default runtime and compares real locale files", async () => {
    ...
    expect(loaderNames).toContain("@pi-harness/core/plugins/i18n-pair");
  ...
  test("loads the production Cleaner tool into the default runtime and removes only excess capsules", async () => {
    ...
    expect(loaderNames).toContain("@pi-harness/core/plugins/cleaner");
    expect(tool).toMatchObject({ executionMode: "sequential", parameters: { additionalProperties: false } });
  ...
  test("loads the production SQL Lens tool into the default runtime and runs a local read-only query", async () => {
    ...
    const runtimeNames = harness.context
      ...
      ?.session.getAllTools()
      .map((candidate) => candidate.name);
    expect(loaderNames).toContain("@pi-harness/core/plugins/sql-lens");
    expect(tool).toMatchObject({ executionMode: "sequential", parameters: { additionalProperties: false } });

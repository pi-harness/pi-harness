...
export async function createTestRuntimeContext(responses: FauxResponseStep[], toolNames: string[] = [], resourceOptions: { noExtensions: boolean; agentDir?: string } = { noExtensions: true }): Promise<{ context: Context; faux: FauxProviderHandle }> {
  ...
  await context.plugin(resourcesPlugin, { noExtensions: resourceOptions.noExtensions, noSkills: true, noPromptTemplates: true, noThemes: true, noContextFiles: true });

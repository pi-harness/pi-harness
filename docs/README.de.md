...
Verwenden Sie `pih` als kanonische Befehlszeilenschnittstelle und `pi-harness`, um die Web-Konsole zu starten.
...
Wichtige Umgebungsvariablen sind `PI_HARNESS_HOST`, `PI_HARNESS_PORT`, `PI_CODING_AGENT_DIR` (`PI_AGENT_DIR` ist ein Kompatibilitätsalias, der nur gelesen wird, wenn `PI_CODING_AGENT_DIR` nicht gesetzt oder leer ist), `PI_HARNESS_HOME`, `PI_HARNESS_PROVIDER` und `PI_HARNESS_MODEL`. Die Standardadresse lautet `http://127.0.0.1:3141`.

Das Profile der Web-Konsole ([`apps/web/profile/cordis.yml`](../apps/web/profile/cordis.yml)) wählt standardmäßig `everyapi/deepseek-v4-flash`. Die Modellauswahl ist fail-closed: Ist dieser Anbieter in dem von `PI_CODING_AGENT_DIR` benannten Agent-Verzeichnis nicht registriert, bricht der Start mit `Pi model is not registered: <provider>/<model>` ab, statt auf einen anderen Anbieter auszuweichen. Nach einer frischen Installation muss der Modellkatalog deshalb zuerst bereitstehen: Installieren Sie die EveryAPI-CLI mit `curl -fsSL https://dl.everyapi.ai/install.sh | bash` (unter Windows `irm https://dl.everyapi.ai/install.ps1 | iex`) und führen Sie `everyapi use pi-harness` aus — das trägt den EveryAPI-Anbieterkatalog in das dauerhafte, von `PI_CODING_AGENT_DIR` benannte Pi-Agent-Verzeichnis ein statt in ein isoliertes eigenes —, oder setzen Sie `PI_HARNESS_PROVIDER` und `PI_HARNESS_MODEL` auf ein Modell, das in diesem Verzeichnis bereits registriert ist. Die in der CLI eingebauten Profiles `default` und `development` wählen über dieselben beiden Variablen dasselbe Paar, dieser eine Schritt registriert den Katalog also für CLI und Web-Konsole zugleich — den EveryAPI-Relay-Schlüssel reicht `everyapi use pi-harness` aber nur an den Prozess weiter, den es selbst startet. Starten Sie die CLI deshalb ebenfalls darüber, als `everyapi use pi-harness -- <pih-Argumente>` mit einem `pi-harness`-Shim auf `PATH`, das pih per exec aufruft, oder geben Sie ihr `PI_HARNESS_PROVIDER` und `PI_HARNESS_MODEL` samt eigenen Zugangsdaten mit.
...
Der ausgewählte Plugin-Katalog sowie Konfiguration, sämtliche HTTP-API-Routen, Limits und Sicherheitsgrenzen stehen in der [englischen Referenz](README.reference.md). Core liefert mehr Plugins aus, als dieser Katalog beschreibt: [`packages/plugins`](../packages/plugins) ist die vollständige Menge, und jedes davon wird wie ein Community-Plugin über das Plugin-Center installiert. Eine frische Installation aktiviert nur die Infrastruktur, also genau das, was in [`apps/web/profile/cordis.yml`](../apps/web/profile/cordis.yml) steht.

...
Usa `pih` come interfaccia a riga di comando canonica e `pi-harness` per avviare la console web.
...
Le variabili principali sono `PI_HARNESS_HOST`, `PI_HARNESS_PORT`, `PI_CODING_AGENT_DIR` (`PI_AGENT_DIR` è un alias di compatibilità letto solo quando `PI_CODING_AGENT_DIR` non è impostata o è vuota), `PI_HARNESS_HOME`, `PI_HARNESS_PROVIDER` e `PI_HARNESS_MODEL`. L’indirizzo predefinito è `http://127.0.0.1:3141`.

Il profile della console web ([`apps/web/profile/cordis.yml`](../apps/web/profile/cordis.yml)) seleziona `everyapi/deepseek-v4-flash` come impostazione predefinita. La selezione del modello è fail-closed: se quel provider non è registrato nella directory agent indicata da `PI_CODING_AGENT_DIR`, l’avvio si interrompe con `Pi model is not registered: <provider>/<model>` invece di ripiegare su un altro provider. Dopo un’installazione nuova prepara quindi prima il catalogo dei modelli: installa la CLI di EveryAPI con `curl -fsSL https://dl.everyapi.ai/install.sh | bash` (`irm https://dl.everyapi.ai/install.ps1 | iex` su Windows) ed esegui `everyapi use pi-harness` — registra il catalogo dei provider EveryAPI nella directory agent Pi durevole indicata da `PI_CODING_AGENT_DIR`, non in una isolata propria — oppure imposta `PI_HARNESS_PROVIDER` e `PI_HARNESS_MODEL` su un modello già registrato in quella directory. I profile `default` e `development` inclusi nella CLI selezionano la stessa coppia tramite quelle stesse due variabili, quindi quell’unico passaggio registra il catalogo sia per la CLI sia per la console web, ma `everyapi use pi-harness` consegna la chiave di relay EveryAPI soltanto al processo che avvia esso stesso. Avvia quindi anche la CLI tramite quello strumento, come `everyapi use pi-harness -- <argomenti di pih>` con uno shim `pi-harness` su `PATH` che esegua exec di pih, oppure assegnale `PI_HARNESS_PROVIDER` e `PI_HARNESS_MODEL` insieme a una credenziale propria.
...
Consulta il [riferimento in inglese](README.reference.md) per il catalogo selezionato dei plugin, la configurazione, tutte le rotte dell’API HTTP, i limiti e la sicurezza. Core include più plugin di quanti il catalogo ne descriva: [`packages/plugins`](../packages/plugins) è l’insieme completo e ognuno si installa dal centro plugin come un plugin della community. Un’installazione nuova abilita solo l’infrastruttura, cioè quanto contenuto in [`apps/web/profile/cordis.yml`](../apps/web/profile/cordis.yml).

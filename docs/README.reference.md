...
> Documentation: [简体中文](README.zh-CN.md) · [日本語](README.ja.md) · [한국어](README.ko.md) · [Español](README.es.md) · [Français](README.fr.md) · [Deutsch](README.de.md) · [Português (Brasil)](README.pt-BR.md) · [Русский](README.ru.md) · [Italiano](README.it.md) · [العربية](README.ar.md)

## Documentation map

- [Requirements and installation](#requirements)
- [Run from source](#run-from-source)
- [Install the published harness](#install-the-published-harness)
- [Run the web console](#run-the-web-console)
...
The localized guides cover the common installation and operation paths. This English README remains the canonical reference for the documented plugin catalog, the configuration schema, the complete API route inventory, limits, and security behavior. The plugin catalog below is a selection: [`packages/plugins`](../packages/plugins) holds every official plugin, and each one is installed from the plugin center like a community plugin rather than shipped with the launcher. [`apps/web/profile/cordis.yml`](../apps/web/profile/cordis.yml) is what a fresh web console enables, and it carries infrastructure only.
...
The web launcher builds the Vite browser bundle, starts the Cordis host, and prints a local URL (by default `http://127.0.0.1:3141`). Set `PI_HARNESS_HOST`, `PI_HARNESS_PORT`, and `PI_CODING_AGENT_DIR` to change the bind address, port, or Pi state directory, or pass `--host <host>` and `--port <port>` to the launcher, which win over the variables for that run. `--provider <id>` and `--model <id>` do the same for `PI_HARNESS_PROVIDER` and `PI_HARNESS_MODEL`, and they exist because `everyapi use pi-harness` exports `PI_HARNESS_MODEL` itself over anything the caller exported and tells the user to pass the model after `--` instead, so `everyapi use pi-harness -- --model <id>` is how a model is chosen through that tool. The browser surface is served by the bundled `@pi-harness/web-app` plugin and talks to the bundled `@pi-harness/api-gateway` over `/api/status`, `/api/session`, `/api/sessions`, `/api/session/new`, `/api/session/open`, `/api/models`, `/api/model`, `/api/files`, `/api/prompt`, `/api/abort`, and the `/api/events` Server-Sent Events stream; [HTTP API routes](#http-api-routes) lists every registered route, including the mutating ones the console uses for plugins, providers, files, and sessions. The launcher refuses non-loopback hosts unless `PI_HARNESS_ALLOW_REMOTE=1` is explicitly set on a trusted network; the API is intended for local use and has no user authentication layer. Because there is no authentication, the web server answers only requests whose `Host` header names loopback, the configured bind host, one of this machine's own addresses or its hostname on a wildcard bind, or a name listed in `PI_HARNESS_ALLOWED_HOSTS`, and rejects cross-site requests whose `Origin` does not match the `Host` header; this blocks CSRF and DNS rebinding, and a reverse proxy must forward the original `Host` header over plain HTTP. A value the launcher rejects — `--port`, `--host`, `--provider` or `--model` without a value, or the environment variable behind the first two — is reported as the single line that names it and exits 2, the usage code `pih` returns for a bad `--profile` or `--config`; every other startup failure exits 1.

The web profile ([`apps/web/profile/cordis.yml`](../apps/web/profile/cordis.yml)) selects `everyapi/deepseek-v4-flash`, stores JSONL sessions under `$PI_CODING_AGENT_DIR/sessions`, and loads Pi resources from the current project and agent directory. The built-in CLI profiles ([`default`](../packages/cli/profiles/default/cordis.yml) and [`development`](../packages/cli/profiles/development/cordis.yml)) select the same pair through the same `PI_HARNESS_PROVIDER` and `PI_HARNESS_MODEL` overrides, so one provisioning step serves both surfaces. Model selection is fail-closed, so a provider that is not registered in the active agent directory aborts startup with `Pi model is not registered: <provider>/<model>` rather than falling back to another provider. Provision the catalog with the EveryAPI CLI — install it with `curl -fsSL https://dl.everyapi.ai/install.sh | bash` (`irm https://dl.everyapi.ai/install.ps1 | iex` on Windows), then run `everyapi use pi-harness` — EveryAPI's tool for this product; `pi-web` is its integration for Pi's own browser UI — which registers the EveryAPI provider catalog in the durable agent directory named by `PI_CODING_AGENT_DIR` rather than in an isolated directory of its own, so the catalog it writes is the one both launchers read. The relay key that catalog refers to is injected only into the process the tool starts, so `pih` runs under it too: `everyapi use pi-harness -- <pih arguments>` with a `pi-harness` shim on `PATH` that execs `pih`, the arrangement [`scripts/pih-local.sh`](../scripts/pih-local.sh) uses for the web console. The provider-agnostic alternative is to point `PI_HARNESS_PROVIDER` and `PI_HARNESS_MODEL` at a model the agent directory already registers: a Pi installation ships a built-in catalog that includes `deepseek/deepseek-v4-flash`, whose credential comes from `DEEPSEEK_API_KEY` or from the `auth.json` beside `models.json` under that directory, which defaults to `~/.pi/agent`.
...
`PI_CODING_AGENT_DIR` is Pi's own variable and the one Pi Harness reads first; `PI_AGENT_DIR` is kept as a compatibility alias and is read only when `PI_CODING_AGENT_DIR` is unset or holds nothing but whitespace, which includes the case where both are exported and the upstream one is empty ([`packages/core/src/agent-directory.ts`](../packages/core/src/agent-directory.ts)). An empty or whitespace-only value in either is treated as unset, and a relative value is resolved against the invocation directory, so `PI_CODING_AGENT_DIR=agent-state` does put the credential store under the current working directory. Neither variable is required to be absolute; pass an absolute path when the store must not follow the directory the command is run from.

The built-in development profile watches the invocation working directory and the launcher automatically supervises a child process with Node's `--expose-internals` flag, which Cordis HMR requires. Cordis performs partial plugin reloads in place and requests a supervised process restart when a framework module changes. The supervisor backs off between restarts and gives up after five restarts in ten seconds so a reload loop cannot fork processes without bound. Production does not expose Node internals. The development profile is also the only built-in profile that mounts a log exporter, and it mounts `@pi-harness/core/plugins/logger`, which writes Cordis log records to stderr at the same warn-level default the console exporter is hardened to, so it keeps the same stdout contract as the profiles that mount none and `pih --profile development "..." | cat` still reads only the assistant's answer.
...
Cordis Loader mutations made at runtime are not persisted back into the profile file, so an activation rollback cannot rewrite the YAML and a hand-edited entry stays exactly as it was written; edit that file directly to change what boots.

A built-in profile is not itself the file that boots. When no `--config` is given, both launchers seed a per-user copy at `<PI_HARNESS_HOME or ~/.pi-harness>/profiles/<name>/cordis.yml` and boot that copy ([`packages/core/src/harness-home.ts`](../packages/core/src/harness-home.ts)); the in-package file linked below is only the template, and it re-seeds the copy for as long as the copy is byte-identical to the seed recorded beside it, so once the copy has been modified, editing the template no longer changes what boots.

The plugin center is the one component that does write a profile, and it writes whichever file the run booted, because it takes the path from `piHarnessLaunch.configPath` ([`packages/api-gateway/src/index.ts`](../packages/api-gateway/src/index.ts), `appendMarketplaceProfile` and `updateMarketplaceProfile` plus the two rollback paths): it inserts a marketplace Loader entry on install, inserts or strips a `disabled: true` flag on toggle, removes the entry on uninstall, and restores the previous file contents when an install's fiber or a toggle's `entry.update` fails. That is the harness-home copy for a built-in profile, and the user's own file for a run started with `--config`, so a project-owned profile booted with `--config` does gain a `marketplace-<plugin id>` entry when a plugin is installed from a console started on it; boot a built-in profile instead when that file must stay as committed. The `/api/marketplace/install`, `/api/plugins/toggle`, and `/api/plugins/uninstall` rows of [HTTP API routes](#http-api-routes) name the same three writes.

## Selected core production plugins

Pi Harness publishes the production-oriented plugins below. None of them is enabled out of the box: install one from the plugin center, or add it to a project-owned profile by name once it is installed. The shipped profiles ([`apps/web/profile/cordis.yml`](../apps/web/profile/cordis.yml) and [`packages/cli/profiles/default/cordis.yml`](../packages/cli/profiles/default/cordis.yml)) enable infrastructure only. Plugin panels expose the latest bounded result and the limits applied by the backend.

This catalog is a selection, not the complete set: [`packages/plugins`](../packages/plugins) holds every official plugin and most of them have no section here. An undocumented plugin runs on exactly the same terms as a documented one once it is enabled — it can register tools the model may call, subscribe to session events, and persist state under `PI_AGENT_DIR` — so read the profile you run and the plugin source rather than reading this catalog as the full behavior of an installation.
...
Mailbox bodies are treated as untrusted collaboration context, not as user approval or authorization. `read_messages` returns and marks at most 25 matching notes per call and reports `nextOffset` for later pages. When `unreadOnly: true`, returned notes leave the unread result set, so continuation intentionally reuses the current offset. Continuation assumes the same session, recipient and filter with no intervening mailbox cleanup; restart at offset `0` after changing that result set. `remove_task`, `remove_member`, and `clear_messages` require `confirm: true`; only completed leaf tasks, unreferenced members, and read notes can be removed. Agent-facing state text is capped at 32 KiB. The backend and browser panels display at most 12 members, 20 tasks, and 5 messages while retaining complete inventory, ready-task, and unread counts.
...
Token Guard has no Agent tool; it observes Pi lifecycle events and asks the active runtime to abort when either configured budget is reached. It applies a 90% context threshold by default and leaves the optional absolute run budget disabled:
...
Publishing is triggered by a push to `main` (including a merged pull request), or manually with `workflow_dispatch`. The `Release packages` workflow runs the complete test, lint, and diff gate, publishes the user-facing `@pi-harness/pi-harness` package alongside the independently installable `@pi-harness/plugin-api`, `@pi-harness/core` and every plugin under `packages/plugins`, skips package versions that already exist, and creates a matching GitHub Release tag. Only the plugins whose directory changed since the previous release tag get a new version, so the rest are skipped as already published. Users normally install only `@pi-harness/pi-harness`; its `@pi-harness/core` dependency is intentionally not bundled, so core plugins can receive a compatible patch release without republishing the launcher. The web app and remaining implementation workspaces are private and are never published, which is why the root `prepack` strips the dependency lists out of the shipped `apps/web/package.json`: the installation is a workspace root, and a manifest naming a package npm cannot fetch makes every npm command run inside the installation fail. The same step strips the API gateway's `devDependencies`, which are the plugins its tests drive: those do resolve, so leaving them in would install two dozen plugins into a harness whose user asked for none of them.
...
- `packages/plugins`: the official plugin packages, one workspace each, published individually and installed from the plugin center

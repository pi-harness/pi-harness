...
Cordis Loader mutations made at runtime are not persisted back into a profile passed with `--config`, so a project-owned profile file is only ever read and hot-refreshed and an activation rollback cannot rewrite it; edit that YAML directly to make changes.

A built-in profile is different, because a built-in profile is not the file that boots. When no `--config` is given, both launchers seed a per-user copy at `<PI_HARNESS_HOME or ~/.pi-harness>/profiles/<name>/cordis.yml` and boot that copy ([`packages/core/src/harness-home.ts`](../packages/core/src/harness-home.ts)); the in-package file linked below is only the template, and it re-seeds the copy for as long as the copy is byte-identical to the seed recorded beside it, so once the copy has been modified, editing the template no longer changes what boots. The plugin center writes into that copy: it inserts a marketplace Loader entry on install, inserts or strips a `disabled: true` flag on toggle, removes the entry on uninstall, and restores the previous file contents when an install's fiber or a toggle's `entry.update` fails ([`packages/api-gateway/src/index.ts`](../packages/api-gateway/src/index.ts), `appendMarketplaceProfile` and `updateMarketplaceProfile` plus the two rollback paths). The `/api/marketplace/install`, `/api/plugins/toggle`, and `/api/plugins/uninstall` rows of [HTTP API routes](#http-api-routes) name the same three writes.

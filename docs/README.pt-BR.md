...
Use `pih` como interface de linha de comando canônica e `pi-harness` para iniciar o console web.
...
As variáveis mais usadas são `PI_HARNESS_HOST`, `PI_HARNESS_PORT`, `PI_CODING_AGENT_DIR` (`PI_AGENT_DIR` é um alias de compatibilidade lido apenas quando `PI_CODING_AGENT_DIR` não está definida ou está vazia), `PI_HARNESS_HOME`, `PI_HARNESS_PROVIDER` e `PI_HARNESS_MODEL`. O endereço padrão é `http://127.0.0.1:3141`.

O profile do console web ([`apps/web/profile/cordis.yml`](../apps/web/profile/cordis.yml)) seleciona `everyapi/deepseek-v4-flash` por padrão. A seleção de modelo é fail-closed: se esse provedor não estiver registrado no diretório de agente indicado por `PI_CODING_AGENT_DIR`, a inicialização é abortada com `Pi model is not registered: <provider>/<model>` em vez de recorrer a outro provedor. Por isso, depois de uma instalação nova prepare primeiro o catálogo de modelos: instale a CLI do EveryAPI com `curl -fsSL https://dl.everyapi.ai/install.sh | bash` (`irm https://dl.everyapi.ai/install.ps1 | iex` no Windows) e execute `everyapi use pi-harness` — ela registra o catálogo de provedores do EveryAPI no diretório de agente Pi durável indicado por `PI_CODING_AGENT_DIR`, e não em um isolado próprio — ou aponte `PI_HARNESS_PROVIDER` e `PI_HARNESS_MODEL` para um modelo já registrado nesse diretório. Os profiles `default` e `development` embutidos na CLI selecionam o mesmo par por essas mesmas duas variáveis, então esse único passo registra o catálogo para a CLI e para o console web ao mesmo tempo, mas `everyapi use pi-harness` entrega a chave de relay do EveryAPI apenas ao processo que ele próprio inicia. Por isso, inicie a CLI também por meio dele, como `everyapi use pi-harness -- <argumentos do pih>` com um shim `pi-harness` no `PATH` que faça exec do pih, ou dê a ela `PI_HARNESS_PROVIDER` e `PI_HARNESS_MODEL` junto com uma credencial própria.

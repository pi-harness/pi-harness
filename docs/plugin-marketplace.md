...
The copy keeps tracking the shipped profile for as long as it is untouched, so a release that adds a bundled plugin still reaches an existing installation. The first marketplace install, or any hand edit, ends that: the copy now carries state the distribution does not know about and is left alone. Deleting it restores the shipped profile on the next boot.

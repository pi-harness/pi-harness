...
Install in the web console is not a copy-to-clipboard action: it POSTs to `/api/marketplace/install` and the gateway performs the change in the running harness. Every entry takes the same path, official or community. The gateway expands declared dependencies into a deterministic dependency-first plan, removes members already present in the loader or profile, and runs one `npm install --save-exact --package-lock=false <packageName>@<version>...` command in the directory that owns the booted profile (this edits that directory's `package.json` and `node_modules`). It writes every missing `marketplace-<id>` entry into that profile in dependency-first order immediately ahead of the runtime entry and inside the group that owns it, then creates loader entries in the same order and waits for each plugin to activate. Position matters because a plugin contributes its tools while it activates and the runtime takes the tool registry when it activates; an entry after the runtime would fail on every later start. If any step fails, every loader entry created by the plan is removed, the profile, `package.json` and `package-lock.json` are restored to their previous contents, and the request fails with 502. Marketplace changes are serialized; a second install, toggle, or uninstall while one is running is rejected with 409.

A plugin that contributes tools is the one case that does not activate on the spot. The runtime holds the tool registry for its whole life and snapshots the tool set when it takes it, so such a plugin cannot join a harness that is already running. The package and the profile entry stay in place, the response carries `restartRequired: true`, the console says the change takes effect after a restart, and the plugin loads normally on the next start. Re-enabling a disabled tool plugin behaves the same way. A dependency present only in the profile is also pending rather than active, so installing its dependent completes the whole profile plan but defers activation until restart.

Because Install executes the package in-process, review the entry's repository before installing, and be aware that a profile passed with `--config` makes that project's `package.json` the install target. Enabling, disabling, and uninstalling go through `/api/plugins/toggle` and `/api/plugins/uninstall`, which edit the same profile file (uninstall also runs `npm uninstall`) and roll back on failure. Pi Harness will fail startup rather than silently ignore a missing or invalid plugin. The marketplace endpoint returns bounded pages, so no single request answers with the whole registry; the console reads the grid one page at a time and prefetches the full catalogue, page by page, only once the Installed or Marketplace page is open, because that is where the metadata behind an installed plugin's card comes from.

Dependency safety continues after installation. Installing a dependent re-enables any disabled requirements in the profile and defers the complete plan to the next start, because the running Runtime may already hold a tool snapshot without them. A marketplace entry installed before dependency metadata was introduced remains actionable when requirements are missing; its button changes to Repair dependencies and runs the same atomic install plan. A dependency cannot be disabled or uninstalled while an installed and enabled plugin requires it, and a dependent cannot be enabled unless all of its requirements are installed, enabled, and loaded in the current runtime. Uninstalling the dependent does not automatically remove its dependencies because other plugins and users may still use them directly.
